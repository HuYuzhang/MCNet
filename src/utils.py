...
def load_vimeo_data(f_name, data_path, image_size, K, T):
    image_size_h = 256
    image_size_w = 448
    seq = np.zeros((image_size_h, image_size_w, K+T, 3), dtype="float32")
    for t in range(0, K+T):
        ...
        # IPython.embed()
        try:
            yuv_img = cv2.cvtColor(img[:image_size_h, :image_size_w, :], cv2.COLOR_RGB2YUV)
        except Exception:
            IPython.embed()
        seq[:,:,t] = transform(yuv_img)
    # Then cal the diff, though we only have one diff
    diff = np.zeros((image_size_h, image_size_w, K-1, 3), dtype="float32")
    for t in range(1, K):
        prev = inverse_transform(seq[:,:,t-1])
